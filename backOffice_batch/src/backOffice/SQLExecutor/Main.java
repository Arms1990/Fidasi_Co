package backOffice.SQLExecutor;
...
public class Main {
	...
	public static final int IDX_ID_COMMAND = 2;
	public static final int IDX_ID_CATENA = 3;
	public static final int IDX_ID_STEP = 4;
	public static final int IDX_ID_ELAB = 5;

	/**
	 * Costruttore del Main SQLExecutor
	...
	public Main(String [] args) throws Exception
	{
		...
		try {
			...
			InputElements.put(backOffice.Constants.FatherXmlNodeName, args[Main.IDX_ID_COMMAND]);
			...
			ConfigManager cm = new ConfigManager(args[Main.IDX_PROP_FILE], 
					args[Main.IDX_XML_FILE], "", InputElements);
			DBManager dbm=null;
			LogManager lm =null;
			/**
			 ...
			 */
			if ( lm.startElab(dbm, cm, "backOffice.SQLExecutor",cm.getCsvFile(false))==0 )
			{
				try
				{
					/**
					 ...
					 */
					if (cm.GetFatherXmlNodeAttribute("RETURN").equals("Y"))
					{
						lm.Insert(dbm, LogManager.INFO, dbm.DoQuery(cm.getChildValuebyName("SQLCommand")), cm);
					}
					else
					{
						dbm.DoCommand(cm.getChildValuebyName("SQLCommand"));
					}
					lm.Insert(dbm, LogManager.INFO, "Comando eseguito correttamente: "+cm.getChildValuebyName("SQLCommand"), cm );
					/**
	...
	{
		System.out.println("--== BackOffice.SQLExecutor v0.5b ==--");
	}
	...
	private int CheckInputs(String [] args)
	{
		...
		params[Main.IDX_ID_COMMAND]="Parametro 4) Identificativo tag XML";
		params[Main.IDX_ID_CATENA]="Parametro 5) ID_CATENA";
		params[Main.IDX_ID_STEP]="Parametro 6) ID_STEP";
		params[Main.IDX_ID_ELAB]="Parametro 7) ID_ELAB (se non inserito verra' autogenerato)";
		if ( args.length < 5 )
		{
		...
		params[Main.IDX_ID_COMMAND]=params[Main.IDX_ID_COMMAND]+Checker.CheckIfNOTNumber(args[Main.IDX_ID_COMMAND]);
