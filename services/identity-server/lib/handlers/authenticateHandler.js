...
var UnauthorizedRequestError = require('../errors/unauthorizedRequestError');
...
function AuthenticateHandler(options) {
  ...
  if (options.roles && undefined === options.addAcceptedScopesHeader) {
    throw new InvalidArgumentError('Missing parameter: `addAcceptedScopesHeader`');
  }

  if (options.roles && undefined === options.addAuthorizedScopesHeader) {
    throw new InvalidArgumentError('Missing parameter: `addAuthorizedScopesHeader`');
  }

  if (options.roles && !options.model.verifyScope) {
...
AuthenticateHandler.prototype.verifyScope = function(accessToken) {
  return promisify(this.model.verifyScope, 2).call(this.model, accessToken, this.roles)
    .then(function(roles) {
      if (!roles) {
...
AuthenticateHandler.prototype.updateResponse = function(response, accessToken) {
  if (this.roles && this.addAcceptedScopesHeader) {
    response.set('X-Accepted-OAuth-Scopes', this.roles);
  }

  if (this.roles && this.addAuthorizedScopesHeader) {
    response.set('X-OAuth-Scopes', accessToken.roles);
  }
};
