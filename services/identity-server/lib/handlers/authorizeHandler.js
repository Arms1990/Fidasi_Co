...
var InvalidScopeError = require('../errors/invalidScopeError');
var UnsupportedResponseTypeError = require('../errors/unsupportedResponseTypeError');
...
var responseTypes = {
  code: require('../responseTypes/codeResponseType'),
  ...
  return Promise.all(fns)
    ...
    .spread(function(expiresAt, client, user) {
      ...
      return Promise.bind(this)
        ...
        .then(function(code) {
          var responseType = new ResponseType(code.code);
...
AuthorizeHandler.prototype.getClient = function(request) {
  ...
  return promisify(this.model.getClient, 2).call(this.model, clientId, null)
    .then(function(client) {
      ...
      if (!client.redirect_uris || 0 === client.redirect_uris.length) {
        throw new InvalidClientError('Invalid client: missing client `redirectUri`');
      }

      if (redirectUri && !_.includes(client.redirect_uris, redirectUri)) {
        throw new InvalidClientError('Invalid client: `redirect_uri` does not match client value');
      }
      return client;
    })
    .catch( (error) => {
      return console.log("AuthorizeHandler - getClient", error);
    });
};
...
AuthorizeHandler.prototype.getResponseType = function(request) {
  ...
  if (!_.has(responseTypes, responseType)) {
    throw new UnsupportedResponseTypeError(`Unsupported response type: '${responseType}' is not supported`);
  }
