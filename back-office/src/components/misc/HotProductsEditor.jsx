...
class HotProductsEditor extends React.Component {
  ...
  async handleSubmission(values) {
    try {
      ...
      return NotificationManager.success(`Hot Products saved successfully.`, `Success!`);
    } catch(e) {
      console.error(e);
  ...
  render() {
    ...
    const { title, action, options, defaultOptions, products, selectedOption } = component;
    return (
      <Card className="my-5">
        <CardBody>
                      ...
                      <Colxx sm={6}>
                        <FormGroup className="has-float-label">
                          ...
                          <Field
                            ...
                            action={action}
                            name={`products`}
                            validate={ (value) => value.length > 0 ? undefined : "Products are required" }
                          />
...
export default HotProductsEditor;