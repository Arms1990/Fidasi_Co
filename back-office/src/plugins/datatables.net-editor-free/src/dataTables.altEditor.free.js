...
import $ from 'jquery';
    ...
    var altEditor = function (dt, opts) {
        if (!DataTable.versionCheck || !DataTable.versionCheck('1.10.8')) {
            throw new Error("Warning: altEditor requires DataTables 1.10.8 or greater");
        }
    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                this.language.modalClose = this.language.modalClose || 'Cancel';
                ...
                var modal = '<div class="modal fade" id="' + modal_id + '" tabindex="-1" role="dialog">' +
                    ...
                    '<button type="button" class="close m-0 p-0" data-dismiss="modal" aria-label="' + this.language.modalClose + '">' +
            ...
            _openEditModal: function () {
                ...
                for (var j in columnDefs) {
                    ...
                    var selectedValue = data.data ? data.data[arrIndex] : data[arrIndex];
            ...
            _editRowData: function () {
                ...
                var rowDataArray = $(`form[name="altEditor-edit-form-${this.random_id}"]`).serializeArray();
            ...
            _openDeleteModal: function () {
                ...
                for (var j in columnDefs) {
                    ...
                    let rowData = adata.data()[0];
                    var selectedValue = rowData.data ? rowData.data[arrIndex] : rowData[arrIndex];


                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        data += "<input type='hidden' id='" + columnDefs[j].name + "' value='" + selectedValue + "'></input>";
                    } else if (columnDefs[j].type.indexOf("password") >= 0) {
                    } else {
                        data += "<div class='form-group row mb-0 mx-0'><label class='mb-0' for='"
            ...
            _deleteRow: function () {
                ...
                var jsonDataArray = $(`form[name="altEditor-delete-form-${this.random_id}"]`).serializeArray();
            ...
            */
            completeColumnDefs: function () {
                ...
                for (var i in dt.context[0].aoColumns) {
                    ...
                    columnDefs[i] = {
                        title: obj.sTitle,
                        name: (obj.data ? obj.data : obj.name),
                        type: (obj.type ? obj.type : 'text'),
                        options: (obj.options ? obj.options : []),
                        excluded: (obj.excluded ? obj.excluded : false),
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                for (var j in columnDefs) {
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        data += "<div class='form-group row align-items-center' id='alteditor-row-" + this._quoteattr(columnDefs[j].name) +"'>";
                        // data += "<div class='col-sm-3 col-md-3 col-lg-3 text-right'>";
                        // data += "<label class='mb-0' for='" + columnDefs[j].name + "'>" + columnDefs[j].title + ":</label></div>";
                        data += "<div class='col-sm-12 col-md-12 col-lg-12'>";

                        // Adding readonly-fields
                        // if (columnDefs[j].type.indexOf("readonly") >= 0 || !columnDefs[j].excluded) {
                        ...
                        // else
                        if (columnDefs[j].type.indexOf("select") >= 0 && !columnDefs[j].excluded) {
                            var options = "";
                        ...
                        // Adding text-inputs and errorlabels, but also new HTML5 typees (email, color, ...)
                            ...
                            if(!columnDefs[j].excluded) {
                                ...
                                data += `<input type="hidden" id="${this._quoteattr(columnDefs[j].name)}" name="${this._quoteattr(columnDefs[j].name)}" value="${this._quoteattr(columnDefs[j].data)}" />`;
                            }
                        }
            ...
            _addRowData: function () {
                ...
                var rowDataArray = $(`form[name="altEditor-add-form-${this.random_id}"]`).serializeArray();
...
//console.log(rowDataArray); //DEBUG
            ...
            _deleteRowCallback: function (response, status, more) {
                    ...
                    var message = '<div class="alert alert-success" role="alert">' +
                        '<strong>' + response || this.language.success + '</strong>' +
                        '</div>';
            ...
            _errorCallback: function (response, status, more) {
                    ...
                    var message = '<div class="alert alert-danger" role="alert">' +
                        ...
                        //  + (error.status == null ? "" : this.language.error.responseCode + error.status) + " "
                         + errstr +
                        '</div>';
    ...
    DataTable.ext.altEditor = altEditor;
